		...
		case pending := <-s.pendingReqCh:
			...
			for {
				...
				if runner != nil {
					...
					// >> Tesla K80
					// Check for multi-GPU conflicts before loading
					if s.hasMultiGPUConflict(gpus) {
						slog.Debug("multi-GPU conflict detected, need to evict")
						runnerToExpire = s.findConflictingRunnerToUnload(gpus)
						break
					}
					// << Tesla K80
...
// >> Tesla K80
// hasMultiGPUConflict checks if loading a model on the target GPUs would conflict
// with already loaded models. This prevents deadlocks on dual-GPU setups like K80.
func (s *Scheduler) hasMultiGPUConflict(targetGpus []ml.DeviceInfo) bool {
	if len(targetGpus) <= 1 {
		return false
	}
		...
		for _, targetGpu := range targetGpus {
			for _, loadedGpu := range loadedRunner.gpus {
				if targetGpu.ID == loadedGpu.ID {
					slog.Debug("multi-GPU conflict detected",
						"target_gpu", targetGpu.ID,
...
func (s *Scheduler) findConflictingRunnerToUnload(targetGpus []ml.DeviceInfo) *runnerRef {
	s.loadedMu.Lock()
