	...
	for _, gl := range gpus.ByLibrary() {
		...
		for _, p := range numParallelToTry {
			...
			if !envconfig.SchedSpread() {
				for _, g := range sgl {
					if ok, estimatedVRAM = predictServerFit([]discover.GpuInfo{g}, f, req.model.AdapterPaths, req.model.ProjectorPaths, req.opts, p); ok {
		...
		for _, p := range numParallelToTry {
			req.opts.NumCtx = req.origNumCtx * p
			if ok, estimatedVRAM = predictServerFit(sgl, f, req.model.AdapterPaths, req.model.ProjectorPaths, req.opts, p); ok {
...
func pickBestPartialFitByLibrary(req *LlmRequest, f *ggml.GGML, gpus discover.GpuInfoList, numParallel *int) discover.GpuInfoList {
	...
	for i, gl := range byLibrary {
		_, estimatedVRAM := predictServerFit(gl, f, req.model.AdapterPaths, req.model.ProjectorPaths, req.opts, *numParallel)
