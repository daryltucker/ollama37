...
func (f GGML) GraphSize(context, batch uint64, numParallel int, kvCacheType string, useFlashAttention ml.FlashAttentionType) (kv []uint64, partialOffload, fullOffload uint64) {
	...
	case "gptoss", "gpt-oss":
		...
		// >> Tesla K80
		// Reduce graph memory estimate for K80s. Upstream assumes padded tensor cores.
		// We use a divisor of 6 and cap at 2.0 GiB.
		// Note from dogkeeper: the original working logic was (gqa * kvTotal / 6).
		// We revert to that exact scalar to avoid over-reservation which causes spilling.
		partialOffload = f.KV().HeadCountMax() / cmp.Or(f.KV().HeadCountKVMin(), 1) * kvTotal / 6
		if partialOffload > 2*format.GibiByte {
			partialOffload = 2 * format.GibiByte
		}
		// << Tesla K80

		if useFlashAttention != 0 { // ml.FlashAttentionEnabled or Auto
	...
	// >> Tesla K80
	// General cap for graph memory if Flash Attention is turned off (which is mandatory for K80s)
	// Upstream estimates can be ~18GB for a 128k context model which OOMs the K80.
	if useFlashAttention == 0 {
		if partialOffload > 2*format.GibiByte {
			partialOffload = 2 * format.GibiByte
		}
		if fullOffload > 2*format.GibiByte {
			fullOffload = 2 * format.GibiByte
		}
	}
	// << Tesla K80
