...
func NewLlamaServer(systemInfo ml.SystemInfo, gpus []ml.DeviceInfo, modelPath string, f *ggml.GGML, adapters, projectors []string, opts api.Options, numParallel int) (LlamaServer, error) {
	...
	if textProcessor != nil {
		return &ollamaServer{llmServer: s, textProcessor: textProcessor, ggml: f}, nil
	} else {
...
func (s *llamaServer) Load(ctx context.Context, systemInfo ml.SystemInfo, systemGPUs []ml.DeviceInfo, requireFull bool) ([]ml.DeviceID, error) {
	...
	// >> Tesla K80
	// 128k context with 512 batch size = ~16GB graph -> OOM.
	// We use a "Smart Product" limit: Batch * Context <= 16,000,000.
	// This allows 32k@512 (Fast) but forces 128k@128 (Safe).
	for _, g := range gpus {
		const K80SafeProduct = 16000000
		currentProduct := uint64(s.loadRequest.BatchSize) * uint64(s.options.NumCtx)

		if strings.Contains(strings.ToUpper(g.Name), "K80") && currentProduct > K80SafeProduct {
			// Calculate safe batch size based on context
			safeBatch := K80SafeProduct / s.options.NumCtx
			if safeBatch < 1 {
				safeBatch = 1
			}

			slog.Info("clamping batch size for K80 to prevent graph OOM (exceeds safe token-product)",
				"original_batch", s.loadRequest.BatchSize,
				"num_ctx", s.options.NumCtx,
				"new_batch", safeBatch)
			s.loadRequest.BatchSize = int(safeBatch)
			// Also update options.NumBatch to be consistent
			s.options.NumBatch = int(safeBatch)
			break
		}
	}
	// << Tesla K80
	...
	// >> Tesla K80
	// Cap estimated graph size for K80s to prevent over-allocation which can lead to OOM
	for _, g := range gpus {
		if strings.Contains(strings.ToUpper(g.Name), "K80") {
			if graphPartialOffload > 2*1024*1024*1024 {
				graphPartialOffload = 2 * 1024 * 1024 * 1024
			}
			if graphFullOffload > 2*1024*1024*1024 {
				graphFullOffload = 2 * 1024 * 1024 * 1024
			}
			break
		}
	}
	...
	blk0, ok := layers["blk.0"]
	if !ok {
		blk0, ok = layers["mm.0"]
	}
	if ok {
		// >> Tesla K80
		// K80s are memory constrained. If we reserve a full layer as buffer, we might spill.
		// Since we already have graph overhead and GPU overhead, let's be aggressive and skip this
		// extra buffer for K80s to maximize layer offloading.
		skipBuffer := false
		for _, g := range gpus {
			if strings.Contains(strings.ToUpper(g.Name), "K80") {
				skipBuffer = true
		...
		if !skipBuffer {
			...
			slog.Debug("skipping layer buffer reservation for K80 to maximize VRAM utilization")
		}
		// << Tesla K80
	} else {
		slog.Warn("model missing blk.0/mm.0 layer size")
	}

	// Assign all the layers to the CPU for now, they will get reassigned later
	for i := range s.ggml.KV().BlockCount() {
		// >> Tesla K80
		// Some models (like gptoss) use 'mm' prefix for blocks instead of 'blk'
		blk, ok := layers[fmt.Sprintf("blk.%d", i)]
		if !ok {
			blk, ok = layers[fmt.Sprintf("mm.%d", i)]
		}
...
func (s *ollamaServer) Load(ctx context.Context, systemInfo ml.SystemInfo, gpus []ml.DeviceInfo, requireFull bool) ([]ml.DeviceID, error) {
	...
	names := []string{}
	for _, g := range gpus {
		names = append(names, fmt.Sprintf("%s (%s, CC %d.%d)", g.Name, g.Library, g.ComputeMajor, g.ComputeMinor))
	}
	slog.Info("loading model", "model layers", s.totalLayers, "requested", s.options.NumGPU, "gpus", names)
	...
	if isK80Any {
		...
		slog.Debug("checking K80 batch clamping", "current_product", currentProduct, "batch", s.loadRequest.BatchSize, "ctx", s.options.NumCtx)

		if currentProduct > K80SafeProduct {
			// Calculate safe batch size based on context
			safeBatch := K80SafeProduct / uint64(s.options.NumCtx)
			if safeBatch < 1 {
				safeBatch = 1
			}
	...
	slog.Debug("ollamaServer.Load layout creation", "batch", s.loadRequest.BatchSize, "ctx", s.options.NumCtx, "kv_cache_type", s.loadRequest.KvCacheType)
	...
	// >> Tesla K80
	// Try loading the model, and if it fails due to memory, retry with smaller batch sizes.
	// This ensures we always try to maximize batch performance while guaranteeing a successful load.
	batchTries := []int{s.loadRequest.BatchSize}
	if s.loadRequest.BatchSize > 256 {
		batchTries = append(batchTries, 256)
	}
	if s.loadRequest.BatchSize > 128 {
		batchTries = append(batchTries, 128)
	}

	var resp *LoadResponse
nextBatchTry:
	for batchIdx, batchSize := range batchTries {
		if batchIdx > 0 {
			slog.Info("retrying model load with smaller batch size for K80", "new_batch", batchSize)
			s.loadRequest.BatchSize = batchSize
			s.options.NumBatch = batchSize

			// Re-calculate layout for the new batch size (which affects graph size estimates)
			gpuLayers, err = s.createLayout(systemInfo, gpus, s.mem, requireFull, backoff)
			if err != nil {
				return nil, err
			}
	...
	nextOperation:
			...
			for {
				...
				if isK80 {
					for i := range resp.Memory.GPUs {
						if resp.Memory.GPUs[i].Graph > 2*1024*1024*1024 {
							resp.Memory.GPUs[i].Graph = 2 * 1024 * 1024 * 1024
						}
					}
					if resp.Memory.CPU.Graph > 2*1024*1024*1024 {
						resp.Memory.CPU.Graph = 2 * 1024 * 1024 * 1024
					}
				}
				// << Tesla K80
				...
				for {
					...
					if !resp.Success && isK80 && batchIdx < len(batchTries)-1 {
						slog.Info("model failed to fit with current batch size on K80, attempting fallback", "batch", s.loadRequest.BatchSize)
						continue nextBatchTry
					}

					if resp.Success {
						clear(pastAllocations) // Reset allocations when we find a working one for this batch
						continue nextOperation
					}
		...
		break // If we finished all operations successfully for a batch, exit the batch try loop
	}
	...
	// >> Tesla K80
	// Ensure the final committed memory is also capped for K80 reporting/layout awareness
	isK80Final := false
	for _, g := range gpus {
		if isTeslaK80(g) {
			isK80Final = true
	...
	if isK80Final {
		for i := range s.mem.GPUs {
			if s.mem.GPUs[i].Graph > 2*1024*1024*1024 {
				s.mem.GPUs[i].Graph = 2 * 1024 * 1024 * 1024
			}
		}
		if s.mem.CPU.Graph > 2*1024*1024*1024 {
			s.mem.CPU.Graph = 2 * 1024 * 1024 * 1024
		}
	}
	// << Tesla K80
...
func isTeslaK80(g ml.DeviceInfo) bool {
	return strings.Contains(strings.ToUpper(g.Name), "K80") ||
		strings.Contains(strings.ToUpper(g.Name), "GK210") ||
		(g.Library == "CUDA" && g.ComputeMajor == 3)
}
...
func (s *llmServer) createLayout(systemInfo ml.SystemInfo, systemGPUs []ml.DeviceInfo, memory *ml.BackendMemory, requireFull bool, backoff float32) (ml.GPULayersList, error) {
	if memory == nil {
		...
		for _, g := range systemGPUs {
			memory.GPUs = append(memory.GPUs, ml.DeviceMemory{
				DeviceID: g.DeviceID,
				Weights:  make([]uint64, s.totalLayers),
				Cache:    make([]uint64, s.totalLayers),
			})
		...
		for i := range gl {
			...
			for j := range memory.GPUs {
				if gl[i].DeviceID == memory.GPUs[j].DeviceID {
					...
					slog.Debug("buildLayout reservation", "gpu", gl[i].ID, "free", gl[i].FreeMemory, "reserved", reserved, "graph", memory.GPUs[j].Graph, "overhead", envconfig.GpuOverhead(), "min", gl[i].MinimumMemory(), "backoff", backoff)
...
func (s *llmServer) verifyLayout(systemInfo ml.SystemInfo, systemGPUs []ml.DeviceInfo, memory *ml.BackendMemory, requireFull bool, gpuLayers ml.GPULayersList, layers []uint64) error {
	// >> Tesla K80
	// Fix: memory.InputWeights are typically already included in layers[totalLayers-1]
	// Adding them again here causes double-counting and prevents loading on tight VRAM GPUs like K80.
	cpuSize := uint64(0)
	// << Tesla K80
	var vramSize uint64
	for _, gl := range gpuLayers {
...
func (s *llmServer) VRAMSize() uint64 {
	...
	// >> Tesla K80
	// InputWeights are already included in g.Size() or CPU.Weights[blocks] from the layering fix.
	// Adding them again here causes double-counting in CLI/UI.
	if noCPULayers {
		mem += s.mem.CPU.Graph
	}
	// << Tesla K80
...
func (s *llmServer) TotalSize() uint64 {
	...
	// >> Tesla K80
	// s.mem.InputWeights are usually already accounted for in s.mem.CPU or s.mem.GPUs
	// during the layering phase, so we don't add them again here to avoid double-counting in CLI/UI.
	mem := s.mem.CPU.Size()
	for _, g := range s.mem.GPUs {
		mem += g.Size()
	}
