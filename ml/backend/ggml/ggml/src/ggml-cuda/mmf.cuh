...
#pragma unroll
    ...
    // >> Tesla K80
    GGML_UNUSED_VARS(col_base, slot_map, y2);
    // << Tesla K80
#ifdef VOLTA_MMA_AVAILABLE
    }
#endif //VOLTA_MMA_AVAILABLE
...
// >> Tesla K80: Fix duplicate template instantiation with proper extern template syntax
#define DECL_MMF_CASE_EXTERN(ncols_dst) \
    extern template void mul_mat_f_cuda<float, ncols_dst>( \
        ...
        const int64_t nsamples_dst, const int64_t stride_sample_x, const int64_t stride_sample_y, const int64_t stride_sample_dst, \
        cudaStream_t stream, const mmf_ids_data * ids_data); \
    extern template void mul_mat_f_cuda<half2, ncols_dst>( \
        const half2 * x, const float * y, const int32_t * ids, float * dst, \
        const int64_t ncols_x, const int64_t nrows_x, int64_t ncols_dst_total, const int64_t stride_row, const int64_t stride_col_y, const int64_t stride_col_dst, \
        const int64_t stride_col_id, const int64_t stride_row_id, \
        const int64_t nchannels_x, const int64_t nchannels_y, const int64_t nchannels_dst, \
        const int64_t stride_channel_x, const int64_t stride_channel_y, const int64_t stride_channel_dst, const int64_t nsamples_x,\
