...
func New(modelPath string, params ml.BackendParams) (ml.Backend, error) {
	...
	// >> Tesla K80
	inputWeightsCounted := make(map[string]bool)
	// << Tesla K80
	...
	createTensor := func(t tensor, bts []C.ggml_backend_buffer_type_t, layer int) *C.struct_ggml_tensor {
		...
		for _, bt := range bts {
			...
			// >> Tesla K80
			// Cap estimated size if the backend reports something crazy (common with fallback buffers)
			actualSize := uint64(C.ggml_nbytes(tt))
			if uint64(size) > actualSize+actualSize/5 { // > 1.2x
				size = C.size_t(actualSize)
			}
			// << Tesla K80

			if layer == -1 {
				// >> Tesla K80
				// InputWeights were sometimes being counted here AND as layer N weights.
				// We prioritize counting them in the layer weights array to avoid double-counting in memory layout checks.
				if btDeviceMemory[bt] != nil {
					// Count all input tensors at layer 'blocks' (the extra layer),
					// but avoid double counting token_embd if it's already counted (it's often reused as output.weight)
					if !inputWeightsCounted[name] {
						btDeviceMemory[bt].Weights[blocks] += uint64(size)
					}
				}

				if !inputWeightsCounted[name] {
					requiredMemory.InputWeights += uint64(size)
					inputWeightsCounted[name] = true
				}
				// << Tesla K80
			} else {
				// >> Tesla K80
				// If this is the output layer and we already counted token_embd.weight here, skip it.
				// This prevents double-counting the same physical tensor that Ollama maps to both input and output.
				if layer == blocks && name == "output.weight" && inputWeightsCounted["token_embd.weight"] {
					// skip
				} else {
	...
	for _, t := range meta.Tensors().Items() {
		...
		case strings.HasPrefix(t.Name, "v.") || strings.HasPrefix(t.Name, "mm.") || strings.HasPrefix(t.Name, "s."):
			// >> Tesla K80
			// If these are actually numbered block tensors (common in some architectures), assign to the correct layer.
			// This prevents lumping all experts/blocks into one giant layer which causes spilling.
			layerIndex := -1
			...
			if layerIndex >= 0 && layerIndex < blocks {
				createTensor(tensor{source: t}, layers[layerIndex].bts, layerIndex)
			} else {
				// Otherwise treat as vision/output tensors
				createTensor(tensor{source: t}, output.bts, blocks)
			}
			// << Tesla K80
