...
func (m BackendMemory) Log(level slog.Level) {
	...
	if sum := sumMemory(m.CPU.Weights); sum > 0 {
		slog.Log(context.TODO(), level, "model weights", "device", m.CPU.Name, "size", format.HumanBytes2(sum))
		total += sum
	} else if m.InputWeights > 0 {
		slog.Log(context.TODO(), level, "model weights", "device", m.CPU.Name, "size", format.HumanBytes2(m.InputWeights))
		total += m.InputWeights
	}
...
func (d DeviceInfo) MinimumMemory() uint64 {
	...
	// >> Tesla K80
	if strings.Contains(d.Name, "K80") {
		return 0
	}
	// << Tesla K80
